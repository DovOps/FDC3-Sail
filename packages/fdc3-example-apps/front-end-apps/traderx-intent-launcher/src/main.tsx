...
type ChannelLike = {
  ...
  broadcast?: (
    context: InstrumentContext & {
      traderxAction?: string
      traderxActionRequestId?: string
    },
  ) => Promise<void>
  getCurrentContext?: (
    contextType?: string,
...
const TraderXIntentLauncher = () => {
  ...
  const syncContextFromChannel = async (agent: any) => {
    ...
    if (typeof currentChannel?.getCurrentContext === "function") {
      ...
      context = await Promise.resolve(
        agent.getCurrentContext("fdc3.instrument"),
      )
    }
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      try {
        const agent =
          ((window as any)?.fdc3?.addContextListener
            ? (window as any).fdc3
            : null) ?? (await getAgent())
        if (!isMounted) {
          return
        ...
        const wildcardListener = await agent.addContextListener?.(
          (context: InstrumentContext) => {
            ...
            console.info(
              "[traderx-intent-launcher] received wildcard context",
              context,
            )
          },
        )
        ...
        const syncInterval = window.setInterval(() => {
          syncContextFromChannel(agent).catch((error) => {
            console.warn(
              "[traderx-intent-launcher] periodic channel sync failed",
              error,
            )
          })
        }, 2000)
  ...
  const broadcastTicketRequest = async (
    ...
    try {
      ...
      console.error(
        "[traderx-intent-launcher] ticket request broadcast failed",
        error,
      )
    } finally {
