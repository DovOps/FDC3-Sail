...
export const PolygonWidget = ({ mode }: { mode: string }) => {
  ...
  useEffect(() => {
    ...
    const registerListeners = async (fdc3: any) => {
      ...
      activeAgent = fdc3
      console.info("[fdc3] Polygon listeners registered", {
        mode: modeProps.name,
      })
    }
    ...
    const syncContextFromChannel = async (fdc3: any) => {
      ...
      try {
        ...
        if (
          !currentChannel ||
          typeof currentChannel.getCurrentContext !== "function"
        ) {
          return
        }
  ...
  useEffect(() => {
    ...
    const call = async () => {
      ...
      const ticker =
        normalizeTicker(state) ?? normalizeTicker(modeProps.initialState)
      if (!ticker) {
        return
      }
